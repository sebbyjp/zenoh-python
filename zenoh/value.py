...
from .keyexpr import IntoKeyExpr, KeyExpr
from .zenoh import _Attachment, _Hello, _QoS, _Reply, _Sample, _SampleKind, _Timestamp, _Value, _ZenohId
    ...
    def __new__(cls, payload: IntoValue, encoding: Encoding = None):
        if encoding is None:
            ...
            if not isinstance(payload, bytes):
                msg = "`encoding` was passed, but `payload` is not of type `bytes`"
                raise TypeError(msg)
...
def _into_bytes(v: str | bytes) -> bytes:
...
class Attachment(_Attachment):
    ...
    @overload
    def get(self, key: str | bytes, default: T) -> bytes | T: ...
    @overload
    def get(self, key: str | bytes) -> bytes | None: ...

    def get(self, key: str | bytes, default=None):
        value = super().get(_into_bytes(key))
        return value if value is not None else default

    def insert(self, key: str | bytes, value: str | bytes):
        return super().insert(_into_bytes(key), _into_bytes(value))

    def update(self, into: IntoAttachment, **kw) -> None:
