...
class Closure(IClosure, Generic[In, Out]):
    ...
    def __init__(self, closure: IntoClosure[In, Out], type_adaptor: Callable[[Any], In] | None = None, prevent_direct_calls=False) -> None:
        _call_ = None
        def _drop_() -> None:
            return None
        ...
        if isinstance(closure, IClosure):
            ...
            msg = "Unexpected type as input for zenoh.Closure"
            raise TypeError(msg)
        adapted = (lambda *args: _call_(type_adaptor(*args))) if type_adaptor is not None else _call_
...
class Handler(IHandler, Generic[In, Out, Receiver]):
    ...
    def __init__(self, input: IntoHandler[In, Out, Receiver], type_adaptor: Callable[[Any], In] | None = None, prevent_direct_calls = True) -> None:
...
class Queue(IHandler[In, None, "Queue"], Generic[In]):
    ...
    def closure(self) -> IClosure[In, None]:
        def call(x) -> None: self.put(x)
        def drop() -> None: self.close()
        return Closure((call, drop))
    ...
    def get_remaining(self, timeout: float | None = None) -> list[In]:
